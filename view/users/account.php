		...
		<div class="module account-form">
			...
			<form class="form" id="account-form" autocomplete="off" action="<?php echo Router::url('users/account/'.$action); ?>" method="post" enctype="multipart/form-data">
				...
				<?php if($action=='profil'||$action==''): ?>
					...
					<?php echo $this->Form->input("account","hidden",array("value"=>$user->account)) ;?>		
					...
					<?php echo $this->Form->SelectNumber('age','Birth year',2006,1950,array('default'=>$user->age,'class'=>'geo-select','style'=>'width:100%;','icon'=>'icon-gift','placeholder'=>"( Your birth year )")) ;?>					
				...
				<?php if($action=='account'): ?>
					...
					<?php echo $this->Form->Radio('account','Compte',array('public'=>'Public','pseudo'=>'Pseudonyme','anonym'=>'Anonyme'),array('default'=>$user->account,'helper'=>"Un compte public peut être vu et recevoir des messages d'autres utilisateurs")) ;?>				
